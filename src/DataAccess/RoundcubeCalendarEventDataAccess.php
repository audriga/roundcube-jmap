...
class RoundcubeCalendarEventDataAccess extends AbstractDataAccess
{
    ...
    public function create($eventsToCreate, $accountId = null)
    {
        ...
        foreach ($eventsToCreate as $c) {
            ...
            if (isset($eventToCreate['calendar'])) {
                $cal = $eventToCreate['calendar'];
                if ($cal instanceof \stdClass) {
                    $keys = array_keys((array)$cal);
                    $eventToCreate['calendar'] = (int)$keys[0];
                } elseif (is_array($cal)) {
                    $eventToCreate['calendar'] = (int)array_key_first($cal);
                }
            }
    ...
    /**
     * Returns IDs of events changed since sinceState.
     */
    public function getChanges($sinceState, $maxChanges = 1000, $accountId = null)
    {
        ...
        $result = $this->db->limitquery(
            "SELECT e.event_id, e.created, e.changed FROM events e
            JOIN calendars c ON e.calendar_id = c.calendar_id
        ...
        foreach ($rows as $row) {
            ...
            // If created and changed are equal, event was just created not modified
